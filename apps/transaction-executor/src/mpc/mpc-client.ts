...
export class MPCClient {
  ...
  async generateThresholdKey(
    ...
    try {
      ...
      this.logger.error(`Failed to generate threshold key: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async initiateSignatureCollection(operation: SignatureRequest): Promise<void> {
    try {
      ...
      const finalSignature = await this.combinePartialSignatures(partialSignatures, operation.threshold);
      
      this.logger.log(`Signature collection completed: ${finalSignature}`);
    } catch (error) {
      this.logger.error(`Failed to initiate signature collection: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async initiateKeyRefresh(
    ...
    try {
      ...
      this.logger.error(`Failed to initiate key refresh: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  private async generatePartialSignature(share: KeyShare, messageHash: string): Promise<PartialSignature> {
    try {
      ...
      this.logger.error(`Failed to generate partial signature: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
