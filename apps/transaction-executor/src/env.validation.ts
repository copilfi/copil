export function validateRequiredEnv() {
  ...
  if (enterpriseSecurityEnabled) {
    ...
    if (!process.env.AWS_ACCESS_KEY_ID) missing.push('AWS_ACCESS_KEY_ID (required for enterprise security)');
    if (!process.env.AWS_SECRET_ACCESS_KEY) missing.push('AWS_SECRET_ACCESS_KEY (required for enterprise security)');
  }
